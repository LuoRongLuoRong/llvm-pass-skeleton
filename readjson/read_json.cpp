...
//    "type": 0
    ...
    int line = files[i]["LOC"].asInt(); // "LOC": 349,
    std::string varname = files[i]["SV"].asString(); // "SV": "m_check_state",
    std::string filename = files[i]["filepath"].asString(); // "filepath": "http/http_conn.cpp"
